...
public class BybitDAO implements CoinDataAccessable, Closeable {
    ...
    /**
     * rest api & websocket에 로그인 요청.
     * BybitDAO 기능 사용을 위해서는 <strong>필수적으로</strong> 가장 먼저 호출되어야 함.
     * @param password 비밀번호
        ...
        if (orderList == null) {
            ...
            for (var coinType: CoinEnum.values()) {
                ...
                dataArray.stream().forEachOrdered(data -> {
                    ...
                    orderList.add(OrderedData.builder()
                                             .coinType(coin)
                                             .tif(tif)
                                             .qty(qty)
                                             .side(sideEnum)
    ...
    public void orderCoin(Orderable order) {
        ...
        JSONObject res = restDAO.placeActiveOrder(order.getSide(), order.getCoinType(), order.getOrderType(), order.getQty(), order.getTIF());
